...
class AdManager {
    ...
    static func createAd(name: String, kind: String, gender: String, age: String, locality: String, animalImage: String, details: String) {
        ...
        ref.child("ads").childByAutoId().child("animal").setValue(["name": name, "breed": kind, "gender": gender, "age": age, "locality": locality, "animalImage": animalImage, "details": details])
    }

    static func uploadMedia(image: UIImage, completion: @escaping (_ url: String?) -> Void) {
        let storageRef = Storage.storage().reference().child("animal.png")
